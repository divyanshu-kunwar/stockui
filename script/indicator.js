...
//Settings for all the indicator begins here . . . . . . . 
    ...
    for(var i=0; i<no_of_controls; i++){
      if(hasButton){
        if(settings.controls[i].btn =="add"){
          list_of_field = settings.controls[i].btn_add;
          document.getElementById(indicator_id+ "btn" + i).addEventListener('click',function(e){
            addMoreField(settings , list_of_field);
          }
          )};
      }   
    }
  ...
  function addMoreField(settings , list_of_field) {
    console.log(list_of_field);
    no_of_controls = Object.keys(settings.controls).length;
      for(var i=0 ; i<list_of_field.length; i++){
        settings.controls[no_of_controls+i] =  settings.controls[list_of_field[i]];
      }
